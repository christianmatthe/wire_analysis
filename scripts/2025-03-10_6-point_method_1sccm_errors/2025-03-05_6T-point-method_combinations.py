...
def multi_point_CEB(pd_dict,
    ...
    for i,z in enumerate(high_pd["z_arr"]):
        ...
        if checksum == len(H2_indicators):
            ...
            # HACK to estimate effect of this error
            p_fit_err =  lambda T : 0*np.sqrt((a_err *Cv(T) * T)**2 + b_err**2)
...
plt.axvline(l_eff, 
        ...
        # + f"{l_eff:.2f}", 
        color = "C2", alpha = 1, ls = "--")    
plt.axvspan(l_eff - err, l_eff + err, label=r"720TC-390C-0A " + r"$l_{\rm eff} = $"
        + f"{l_eff:.2f}" + r"$\pm$" + f"{err:.2f}", 
