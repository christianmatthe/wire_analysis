...
ax1= plt.gca()
T_scale_lst = [0.98,1,1.02]
for j,T_scale in enumerate(T_scale_lst):
    T_lst = [T_scale * TC_to_T_Hack(TC) for TC in TC_lst]
    print(T_lst)
    ...
    if j == 1:
        ax1.errorbar(T_lst, ac_lst, yerr = comb_err, fmt = ".",
            label = r"p_and_$l_{\rm{eff}}$_errs",
            ms = 18, lw = 5, color = "C0"
            )
        ax1.errorbar(T_lst, ac_lst, yerr = ac_err_lst, fmt = ".",
            label = r"p_stat_errors",
            ms = 18, lw = 5, color = "C1"
            )
    else:
        ax1.errorbar(T_lst, ac_lst, yerr = comb_err, fmt = ".",
            label = r"p_and_$l_{\rm{eff}}$_errs",
            ms = 18, lw = 5, color = "C0", alpha=0.5
            )
        ax1.errorbar(T_lst, ac_lst, yerr = ac_err_lst, fmt = ".",
            label = r"p_stat_errors",
            ms = 18, lw = 5, color = "C1", alpha=0.5
            )
...
dpi = 300
plt.savefig(out_dir + "alphaE_perr_l_eff_err_T_prop"
            + '.{}'.format(format_im),
