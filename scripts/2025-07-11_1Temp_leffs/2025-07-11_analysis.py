...
filename_list = [indicator + f"_{TC_lst[i]}TC"+".json" 
...
#Plot  all individual datasets:
for i,filename in enumerate(filename_list):
    plotname = f"{indicator_list[i]}_{TC_lst[i]}TC_P_over_z"
    fig = plt.figure(0, figsize=(8,6.5), dpi =300)
    ax1=plt.gca()
    x_label = r"$z_{pos}$ [mm]"
    print("filename:", filename)
...
def make_lists_1T(pd):
    #very basic background subtractionn without temperature axis alignment
    # align data on z-axis (of most sparese data set)
    z_lst = pd["z_arr"]
    p_excess_lst = pd["p_arr"]
    p_excess_err_lst = pd["p_err_arr"]
...
for i, indicator  in enumerate(indicator_list):
    z_lst, p_excess_lst,p_excess_err_lst = (
        make_lists_1T(pd_dict[indicator]
        ))

    filename = f"{indicator}_{TC_lst[i]}TC.json"
    beamfit = wa.Beamfit(
        run_dict_path = work_dir + filename)
    # HACK to change out dir without editing the files
    # TODO If you do this implement feature to retroactively change out_dir in
