...
indicator_list = ["720TC",  "390TC", "0A", "715TC_new"]
...
z_lst, ceb_lst, ceb_err_lst, p_excess_lst,p_excess_err_lst = three_point_CEB(
    pd_dict["0A"],pd_dict["390TC"],pd_dict["715TC_new"])
z_lst, ceb_lst_ac_H_lit,ceb_err_lst_ac_H_lit, p_excess_lst,p_excess_err_lst = three_point_CEB(
    pd_dict["0A"],pd_dict["390TC"],pd_dict["715TC_new"],ac_H=0.65, gamma_H=0.03)
...
print("custom penumbra fit wait ~??min")
beamfit.custom_fit(z_arr=np.asarray(z_lst).flatten(),
                         p_arr=np.array(pd_dict["715TC_new"]["p_arr"]).flatten()
                        , p_err_arr = np.array(
                            pd_dict["715TC_new"]["p_err_arr"]).flatten(),
                plotname= "custom_fit_raw_data"
                        )
